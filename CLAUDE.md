# Scoria — 研究ソフトウェア開発向け Claude Code 拡張ジェネレータ

研究ソフトウェア開発（RSE。ADR-0030）の作業を入力すると、Claude Code の**スキル・エージェント・
プラグイン**をウィザードで生成し ZIP でダウンロードできる Web アプリ。開発の進め方は deb 由来の
土台に従う（ADR-0024）。設計判断は docs/adr/ を参照（覆さない。覆すなら新 ADR）。
